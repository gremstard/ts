...
// impv (https://github.com/.../impv, lives locally at ~/impv) renders video
// as colored terminal blocks with synced audio via ffmpeg + afplay. `ts`
// just hands it a file or URL — no reason to maintain a second
// frame-rendering pipeline in JS alongside impv's Python one.
const IMPV_PATH = path.join(os.homedir(), "impv", "impv");

export async function previewVideo(url) {
  if (!fs.existsSync(IMPV_PATH)) {
    throw new Error(
      `impv not found at ${IMPV_PATH} — it's the terminal video renderer this depends on.`
    );
  }
  ...
  if (stream) {
    await runImpv(stream.url, stream.headers);
    return;
  }
  ...
  try {
    await runImpv(videoPath, {});
  } finally {
    fs.rmSync(workDir, { recursive: true, force: true });
  }
}

function runImpv(target, headers) {
  return new Promise((resolve, reject) => {
    const headerArgs = Object.entries(headers).flatMap(([k, v]) => ["-H", `${k}: ${v}`]);
    const proc = spawn(IMPV_PATH, [target, ...headerArgs], { stdio: "inherit" });
